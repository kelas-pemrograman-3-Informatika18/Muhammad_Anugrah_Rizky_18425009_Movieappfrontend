          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              ...
              <q-td key="status" :props="props">
                ...
                <q-badge v-else color="warning" class="q-pa-sm">
                  Sudah diterima pembeli
                </q-badge>
              </q-td>
              <q-td key="aksi" :props="props">
                <q-btn label="Detail" @click="openDetail(props.row)" color="primary" unelevated/>
                <q-btn :disabled="props.row.status !== 1" label="Konfirmasi" @click="confirm(props.row._id)" color="green" class="q-ml-sm" unelevated/>
              </q-td>
            </q-tr>
          </template>
        </q-table>
...
export default {
  ...
  methods: {
    ...
    confirm (id) {
      this.$q.dialog({
        title: 'Confirm',
        message: 'Apakah anda yakin mengkonfirmasi order ini?',
        cancel: true,
        persistent: true
      }).onOk(() => {
